...
- 系数冷启动为 600（mac 700）先验，完成 1~2 个 ≥300 token 的调用后收敛；管道静默调用不入样（见 key-rules #5）。**系数估计器（`cal_estimate`，2026-09-24 起）**：生效系数不再取「最近 5 样本普通中位数」，改为 **10 样本新近加权中位数**——半衰期 3 个样本（量级突变后 2~3 个新样本即跟上，不输旧 5 窗的自适应），两遍法温和修剪（以未修剪加权中位数为心，[0.4, 2.5] 倍外的离群样本不进加权，单条污染样本无法大幅撼动系数），窗口不足 3 个样本时向先验线性收缩（保持冷启动保护）。依据：本机 258 个真实校准样本重放（`python scripts/cal_bench.py`）——B/token 逐调用天然波动 p25~p75 = 466~743、lag-1 自相关 ≈0.50，误差地板 ≈20%；新估计器预测下一调用的相对误差中位数 24.3%→**22.1%**、p75 49.9%→47.8%，合成基准（liveio 测试 `benchmark_*`：稳态 AR(1)/突变收敛/离群尾部）同向。已重放否决：分模型校准（GLM-5.3 与 Flash 样本分布一致）、EWMA/全局收缩/更大均窗。**系数样本持久化**：队列随变化（入样/重校准）落盘 `~/.zcode/speed-panel-cal.json`（`updated_ms` + `samples`），重启/热重启热启动（生效系数 = 恢复后队列经同一估计器输出，与校准路径同口径；值域外样本拒收），无文件/损坏/超 14 天（模型换代后旧样本即过期噪声）回先验重新收敛——此前每次重启从先验 600 起步，系数真值偏离先验的会话（实测高速档 ~160）重启后读数偏低 2~3 倍、收敛约 25 分钟。重校准后队列 [先验] 同样落盘，"重校准意图"跨重启保留。
