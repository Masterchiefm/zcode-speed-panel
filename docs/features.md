...
- 系数冷启动为 600（mac 700）先验，完成 1~2 个 ≥300 token 的调用后收敛；管道静默调用不入样（见 key-rules #5）。**系数估计器（`cal_estimate`，2026-09-24 二轮修订）**：生效系数 = **16 样本新近加权中位数 + AR1 新近收缩**——半衰期 3 个样本（量级突变后 2~3 个新样本即跟上），两遍法温和修剪（以未修剪加权中位数为心，[0.4, 2.5] 倍外的离群样本不进加权），剪后向最新样本做一步对数收缩（ρ=0.3，比值钳 [0.5, 2]：样本 log 空间 lag-1 自相关 ≈0.50 的纯 AR(1) 结构下的一步最优预测，单样本影响有界 ≤ ×1.23——离群只拉动一拍、突变首样本先手），窗口不足 3 个样本时向先验线性收缩（保持冷启动保护）。依据：本机 262 个真实校准样本重放（`python scripts/cal_bench.py`，分块验证 ρ 外推稳定）——B/token 逐调用天然波动 p25~p75 = 466~740、误差地板 ≈20%；误差中位数 24.3%（旧 5 样本普通中位数）→ 21.3%（10 样本窗）→ **20.7%**，p75 47.8%→**41.5%**、均值 37.1%→**34.1%**，合成基准（liveio 测试 `benchmark_*`：稳态 AR(1)/突变收敛/离群有界拖动）同向。已重放否决：分模型校准（两轮：GLM-5.3 与 Flash 分布一致；deepseek 样本 n=3 太薄）、字节 ≈ a×token + c×时长两参数模型、EWMA/全局收缩/更大均窗/eff 加权。**系数样本持久化**：队列随变化（入样/重校准）落盘 `~/.zcode/speed-panel-cal.json`（`updated_ms` + `samples`），重启/热重启热启动（生效系数 = 恢复后队列经同一估计器输出，与校准路径同口径；值域外样本拒收），无文件/损坏/超 14 天（模型换代后旧样本即过期噪声）回先验重新收敛——此前每次重启从先验 600 起步，系数真值偏离先验的会话（实测高速档 ~160）重启后读数偏低 2~3 倍、收敛约 25 分钟。重校准后队列 [先验] 同样落盘，"重校准意图"跨重启保留。
...
- 精度：达标调用 `pred/true` 应在 0.8~1.25（实测 1.00~1.05）；对账命令 `python scripts/live_vs_true.py`。系数估计器的精度基准用 `python scripts/cal_bench.py`（真实校准样本重放，`--models` 联库按模型分组、`--corr` 看样本自相关；改估计器前后必须对表，见 key-rules #5）。
