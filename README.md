...
- **设置：模块显隐与排序**（顶栏 ⚙ 齿轮按钮）：勾选要在完整面板显示的模块并用 ↑↓ 调整上下顺序（列表从上到下 = 面板从上到下，即时生效并跨重启记住）。默认显示 仪表盘 / 网速监控 / 近 15 分钟输出速度，**快照防护与上传记录默认隐藏**——需要时再勾选开启；「恢复默认」一键还原。并发任务卡跟仪表盘走（不单列）；模块开关不影响悬浮窗与状态栏。
